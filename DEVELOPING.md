...
- **ALL toggles inert (menu *and* hotkey) but muting still shows and the options page still opens** → a stale **Muted Boilerplate** install is co-resident with Hush. The rename changed the VSIX Identity (`MutedBoilerplate.VS.…` → `Hush.VS.…`) but kept the package GUID (`7b4b1d6c-…10`) and command-set GUID (`…11`), so VSIXInstaller installs the two side-by-side instead of upgrading, and both claim the same command table — command routing goes ambiguous and dispatches nowhere. Muting survives because the classifiers are MEF (aggregated, not package-loaded); the options page survives because `DialogPage` is created on demand. VS even loads the *old* `MutedBoilerplate.VS.dll` for the shared package GUID, whose `MuteStateService` is a different type in a different assembly than Hush's — so its toggles mutate a state object the Hush classifier never reads. Fix: uninstall **Muted Boilerplate** (Extensions → Manage Extensions, or delete its `Extensions\<hash>\` folder with VS closed and clear `ComponentModelCache` + `extensions.*.cache`). Check **every** hive — the duplicate lands in each VS that ever had both (2026 `18.0_*` and 2022 `17.0_*`). Grep for it: `MutedBoilerplate.VS.pkgdef` under `%LOCALAPPDATA%\Microsoft\VisualStudio\*\Extensions\`.
