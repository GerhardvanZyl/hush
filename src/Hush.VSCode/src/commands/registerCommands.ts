...
export function registerCommands(
  ...
  reg('hush.toggleTelemetry',  () => state.toggle(BuiltInCategoryKeys.Telemetry));
  reg('hush.toggleLogging',    () => state.toggle(BuiltInCategoryKeys.Logging));
  reg('hush.toggleSignature',  () => state.toggle(BuiltInCategoryKeys.Signature));
  reg('hush.toggleGuards',     () => state.toggle(BuiltInCategoryKeys.Guards));
  reg('hush.toggleAll',        () => state.toggleAll());
  reg('hush.toggleExclusions', () => state.toggleExclusions());

  for (let slot = 1; slot <= 8; slot++) {
    reg(`hush.toggleUser${slot}`, async () => {
