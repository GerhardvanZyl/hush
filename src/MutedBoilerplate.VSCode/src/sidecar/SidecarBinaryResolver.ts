 ...
 *   out/sidecar/<rid>/MutedBoilerplate.VSCode.Sidecar[.exe]
 *
...
export function resolveSidecarPath(extensionRoot: string, overridePath?: string): string {
  if (overridePath && overridePath.trim().length > 0) {
    if (!fs.existsSync(overridePath)) {
      throw new Error(`Configured mutedBoilerplate.sidecarPath does not exist: ${overridePath}`);
    }
    return overridePath;
  }
  ...
  const exeName = process.platform === 'win32'
    ? 'MutedBoilerplate.VSCode.Sidecar.exe'
    : 'MutedBoilerplate.VSCode.Sidecar';

  const sidecarBin = path.join(extensionRoot, '..', 'MutedBoilerplate.VSCode.Sidecar', 'bin');
  const candidates = [
