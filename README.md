# vs-ext-muted-boilerplate

A Visual Studio extension that visually mutes boilerplate code (telemetry calls, logging, method signatures, etc.) so it fades into the background while the meaningful logic stays prominent.
...
- `src/MutedBoilerplate.Core/` — `netstandard2.0` rule model, matchers (Roslyn call, signature, regex, identifier), `IMuteSpanProvider`, exclusion evaluator. No VS dependencies.
- `src/MutedBoilerplate.VS/` — `net472` VSIX. MEF-exports the classifier + outlining tagger, hosts the options page and command set, bridges `ITextBuffer` to a Roslyn `Document` via the live workspace.
- `tests/MutedBoilerplate.Core.Tests/` — xUnit tests for matchers, exclusion evaluator, rule-set round-trip, end-to-end span provider.
...
Override foreground/typeface in **Tools → Options → Fonts and Colors** under the `Muted *` entries. Toggle auto-collapse and pick a custom rules file in **Tools → Options → Muted Boilerplate**.
...
The VSIX project (`src/MutedBoilerplate.VS`) builds the assembly under either CLI MSBuild or VS, but **producing the `.vsix` container reliably requires opening `vs-ext-muted-boilerplate.sln` in Visual Studio 2022/2026 with the *Visual Studio extension development* workload installed and building from there**. Pressing F5 launches the experimental hive with the extension deployed.

## Reuse seam for a future VS Code extension

`MutedBoilerplate.Core` deliberately depends only on `Microsoft.CodeAnalysis.CSharp` and `System.Text.Json`. The same `IMuteSpanProvider`, `RuleSet`, and `MuteState` types can be hosted in a `dotnet`-based JSON-RPC sidecar process that a VS Code extension speaks to over stdio, exposing matches as `setDecorations` calls and folding ranges via a `FoldingRangeProvider`.
